...
Java Static Extract provides a small SER rule language and a JDT runtime for
extracting static facts from Java source code.
...
把所有框架解析逻辑都写死在引擎里，会让引擎很难扩展。Java Static Extract 把问题拆成两层：

- SER rules describe what code shape to find and what fields to build.
- The JDT runtime executes those rules, traces Java values, and returns a stable
  field map.
...
Java Static Extract can:

Java Static Extract 当前可以做这些事：
...
such as Spring MVC and RestTemplate live in `java-static-extract-examples`.

runtime 模块默认不内置框架规则。Spring MVC、RestTemplate 这类规则放在 `java-static-extract-examples` 里作为示例。

## Modules

项目分为五个模块，分别负责规则模型、JDT 执行、agent 工作流、CLI 和示例规则。
...
```bash
mvn -pl java-static-extract-cli -am package
```
...
```text
java-static-extract-cli/target/appassembler/bin/java-static-extract
```
...
```bash
java-static-extract-cli/target/appassembler/bin/java-static-extract init \
  --project /my-project

java-static-extract-cli/target/appassembler/bin/java-static-extract try \
  --project /my-project \
  --file /my-project/src/main/java/demo/Api.java \
  --rule /my-project/.ser/generated/http.ser

java-static-extract-cli/target/appassembler/bin/java-static-extract diagnose \
  --project /my-project \
  --file /my-project/src/main/java/demo/Api.java \
  --rule /my-project/.ser/generated/http.ser

java-static-extract-cli/target/appassembler/bin/java-static-extract run \
  --project /my-project \
  --rule /my-project/.ser/generated/http.ser \
